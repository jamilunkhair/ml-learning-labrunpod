...
def handler(job):
    ...
    try:
        ...
        clean_out = _clean_console(stdout.getvalue())[-50000:]
        if not clean_out:
            clean_out = f"Eksekusi selesai melalui Runpod worker {WORKER_VERSION}. Tidak ada output print(), tetapi summary/plot tetap dikirim jika tersedia."
        return {
            "ok": True,
